...
function(_Prerequisite_Execute_Immediate name step command working_dir
  ...
  _Prerequisite_Debug("  STAMP_FILE: ${stamp_file}")
  _Prerequisite_Debug("  WORKING_DIR: ${working_dir}")
  _Prerequisite_Debug("  COMMAND: ${substituted_command}")
  if(EXISTS "${stamp_file}")
    _Prerequisite_Debug("  STAMP exists before execution: YES")
  else()
    _Prerequisite_Debug("  STAMP exists before execution: NO")
  endif()
  ...
  if(NOT result EQUAL 0)
    # Clean up stamps for this and subsequent steps (single-stamp architecture)
  ...
  # Create stamp file only on success
  _Prerequisite_Debug("  Creating stamp: ${stamp_file}")
  file(TOUCH "${stamp_file}")
  if(EXISTS "${stamp_file}")
    _Prerequisite_Debug("  Stamp created successfully")
  else()
    _Prerequisite_Debug("  WARNING: Stamp creation failed!")
  endif()
endfunction()
...
function(_Prerequisite_Create_Wrapper_Script name step command_list stamp_file out_execute_command)
...
# Create build-time target for a step using single stamp architecture
function(_Prerequisite_Create_Build_Target name step command working_dir stamp_file dependencies)
  ...
  # Single stamp architecture: stamp marks execution complete
  ...
  _Prerequisite_Debug("_Prerequisite_Create_Build_Target(${name}, ${step}) - Single stamp architecture")
  _Prerequisite_Debug("  STAMP_FILE: ${stamp_file}")
  ...
  _Prerequisite_Create_Wrapper_Script("${name}" "${step}" "${substituted_command}" "${stamp_file}" execute_command)
  ...
  # Create force target (removes stamp to force rebuild)
...
function(_Prerequisite_Process_Single_Step name step is_configure_time previous_stamp_file out_stamp_file)
  ...
  # Set up stamp file path (single stamp architecture)
  ...
  if(previous_stamp_file)
    ...
    _Prerequisite_Debug("  Adding previous stamp to deps: ${previous_stamp_file}")
  endif()
  if(uses_file_deps)
